<template>
    <div class='category-list'>
        ...
        <el-table :data="tableData" style="width: 95%" size="mini">
            <el-table-column label="分类ID" width="250">
    ...
    export default {
        ...
        methods: {
            ...
            async handleEdit(index, row) {
                ...
                let _id = row._id
                let res = await this.$get('/category-one', { _id })
                console.log(res)
                this.ruleForm = res
                this.drawer = true
            },
            async handleDelete(index, row) {
                await this.$con_f("确认删除该分类？")

                let { success, msg } = await this.$post('/category-delete', row)
            ...
            submitForm(formName) {
                this.$refs[formName].validate(async (valid) => {
                    if (valid) {
                        if (this.ruleForm.name === this.categoryName) {
                            this.$msg_w('未对分类名称进行修改')
                            return
                        }

                        let { success, msg } = await this.$post('/category-update', this.ruleForm)
                        // 判断是否修改成功
                        if (success) {
                            this.getTableData();
                            this.$msg_s(msg)
